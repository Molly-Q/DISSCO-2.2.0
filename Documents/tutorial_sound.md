...
Create your first audio file, then learn to change its pitch, timing, sound color, and effects. No programming or MIDI keyboard is needed.

**Start with sections 1–9:** build and hear one five-second sound. **Continue through section 10:** change one setting at a time and listen as it becomes a five-sound piece. **Choose sections 11–23 as needed:** they are independent experiments and references, not prerequisites for finishing your first piece. Save at any completed checkpoint and return later.

**LASSIE** is the editor; DISSCO's **CMOD** and **LASS** engines render the sound. **Save** stores the project. **Project > Run**, with **Sound Synthesis** checked, creates an audio file. 

Two projects provide starting and recovery points:

| Project | What you will hear | Use |
| --- | --- | --- |
| [first_sound.dissco](examples/tutorial_sound/first_sound.dissco) | One centered sound fading in and out | End of section 9; experiments 11–19 and 23 |
| [tutorial_sound.dissco](examples/tutorial_sound/tutorial_sound.dissco) | Five sounds with varied pitches, stereo movement, and reverb | End of section 10; experiments 20–22 |
...
Screenshots show LASSIE at high resolution. Open an image to view it at full size; window styling can vary by operating system.
...
- [7. Draw an loudness/amplitude envelope](#7-draw-an-amplitude-envelope)
- [8. Create Spectrum sp1](#8-create-spectrum-sp1)
- [9. Connect and hear your first sound](#9-connect-and-hear-your-first-sound)
- [10. Turn one sound into a short piece](#10-turn-one-sound-into-a-short-piece)

**Optional experiments — start each section from its stated project**

- [11. Change pitch and iphase carrier phase](#11-change-pitch-and-carrier-phase)
- [12. Set loudness/amplitude](#12-set-loudness)
...
- [17. Shape the sound with filters](#17-shape-the-sound-with-filters)
...
- [21. Construct a longer piece](#21-arrange-a-longer-piece)
...
**Optional first listen:** open the supplied [first_sound.dissco](examples/tutorial_sound/first_sound.dissco) from its own working folder and follow [Run and listen](#run-and-listen). Hearing its single sound confirms the installation and playback path before you build anything. Then choose **New Project** in section 3 to build your own version.

## 2. Understand the project structure

An **event** describes object that exists in time. A **parent** creates **children** events. This particular project uses three objects in Objects List and two envelopes (curves) in Envelope Library. LASSIE creates Top 0 when you make a new project; you will add Bottom s1, Spectrum sp1, and the envellopes.
...
| **Spectrum sp1** | Describes the ingredients inside each sound. Our first spectrum has one sine-wave partial (overtone). |
 child can start before the previous one ends
...
| **Envelope 2** | A horizontal curve with value 1. When scaled to 0.5, it keeps the first sound centered between two speakers. |
...
A **partial** (overtone) is one component of a sound. Adding partials changes the **timbre** ("sound color"). An **envelope** is a curve (function) that describes - in most cases - how a value changes over time.

## 3. Create a project

1. Click **New Project**.
2. Choose a working folder such as Home or Documents, not the aplication bundle, DISSCO source directory or installation directory.  Enter `first_sound` as the filename, and click **Save**. LASSIE creates a project folder with the same name and places `first_sound.dissco` inside it.
3. In **Project Properties**, enter the values below.

| Setting | Value |
| --- | --- |
| Project Title | `first_sound` |
...
Keep the default tempo and time signature. They become relevant when you use EDU units later.
...
The first sound starts at `0` and lasts five seconds. 
<!----------------------
Random timing comes after you hear it.
		--------------------------->
...
Before closing the library, make the curve for the first sound's stereo position:
...
When using an envelope to controll the loudness/amplitude of a partial, it has to start with x=0; y=0 and end with x=1; y=0 in order to avoid distortions (clicks or pops) caused by abrupt changes.
...
**Scaling Factor** multiplies the envelope's Y values. A factor of `1` keeps the curve unchanged. A factor of `0` makes this partial silent. The single partial gives us a simple sound whose pitch will be set in Bottom s1.

**Setup check:** Spectrum sp1 has one partial row using EnvLib Number `1`, Scale `1`. Connect it to Bottom in section 9.

## 9. Connect and hear your first sound
...
Envelope 2 stays at 1; multiplying it by 0.5 gives a constant position of 0.5. With the usual left/right channel order, this places the sound in the center. Spatialization needs this complete function even for a stationary sound.
...
6. In Finder or your system's file manager, open the project folder containing your `.dissco` file, then **SoundFiles**. Open the newest AIFF in an audio player or sound editor. LASSIE does not automatically play it.

! Seed dialog](images/tutorial_sound/13-random-seed.png)
...
The first output is named `first_sound_0.aiff`; later runs increase the suffix. Output names follow the project filename. Check the modification time to identify the newest result.

**Listen checkpoint 1 — your first sound.** Listen at a comfortable playback volume. One steady pitch should begin fading in immediately, reach its peak halfway through its five-second duration, then fade out. It should stay in the center. If you hear nothing, check Bottom's count `1`, start `0`, the two child-table connections, and the two EnvLib references in the Ready to Run paragraph. If no file appears, use [No audio file appears](#no-audio-file-appears). Do not add more settings until this sound works.

Click **Save** to keep your completed `first_sound.dissco`. Keep this file and its audio as your starting point for later experiments. If it stops working, the supplied [first_sound.dissco](examples/tutorial_sound/first_sound.dissco) is a recovery point.
...
## 10. Turn one sound into a short piece

1. With your completed first sound open, choose **File > Save As**. Create or select a separate folder and save as `tutorial_sound.dissco`. Unlike New Project, Save As writes directly into the selected folder.
2. In **Project > Set Properties**, change **Project Title** to `tutorial_sound`, accept, and Save.

You are now editing the copy; `first_sound.dissco` stays unchanged. The checkpoints below build on each other. Use [Run and listen](#run-and-listen) each time.
...
With the usual channel order, position `0` is right, `0.5` is center, and `1` is left. Here the triangle controls position: its Y = 0 means right, not silent. Partial 1 still uses that same triangle separately for amplitude.

**Listen checkpoint 3 — left, right, left.** Run. On headphones or two speakers, the sound should move toward the left as it grows, then back toward the right as it fades. The very ends are quiet because the amplitude also reaches zero. If it stays centered, check EnvLib Number `1`, Scale `1`, and that playback is stereo rather than mono. You can continue with a mono playback device, but it cannot demonstrate the left/right movement.
...
Make sure the random Start Time function is set before increasing the count. Otherwise, five children with Start Time `0` would begin together. A start at second 25 plus a five-second duration fits within the 30-second Bottom event. Children and their entire duration have to fit within the duration of the parent.

**Listen checkpoint 5 — one sound becomes a texture.** Run. The engine now creates five sounds at the same 440 Hz pitch with varied starting times. They can overlap, so you may hear blended swells rather than five separate attacks. There can now be silence before the first sound. If you need to verify the count, open the generated `.particel` text report beside the project (for example, `tutorial_sound.particel`) and check its five `+-Sound` entries. If all attacks coincide, reopen Child Start Time and check the Random bounds.
...
**Listen checkpoint 6 — your first short piece.** Run. You should now hear varied pitches, five fading sounds that may overlap, stereo movement, and a room reverberation (tail). If the pitches remain fixed, check Equal Tempered and reopen its RandomInt function. Random choices can repeat a pitch; five different pitches are not guaranteed. Adding pitch randomness can also change the later start-time draws, so this comparison demonstrates pitch variety rather than identical timing.
...
| --- | --- | --- |
| 11–19 and 23 | [first_sound.dissco](examples/tutorial_sound/first_sound.dissco) | One 440 Hz sound, one partial, centered, no reverb or modifiers |
| 20–22 | [tutorial_sound.dissco](examples/tutorial_sound/tutorial_sound.dissco) | Five sounds, random timing and pitch, stereo movement, reverb |
...
**Start:** a fresh first-sound copy. **Work in:** Bottom s1 > Frequency.
...
| --- | --- | --- |
| **Equal Tempered** | Integer-number semitones above C0 | `48` = C4, about 261.63 Hz. Add `12` for one octave. |
| **Fundamental** | Fundamental Frequency in Hz × positive integer Partial Number | `220` × `2` gives a pitch of 440 Hz. |
...
| --- | --- | --- |
| **Hertz** | Frequency in cycles per second | `440` = A4. |
| **Power of Two** | Octaves above C0; an increase of `1` doubles frequency | `4` = C4; `4.5` is six semitones above C4; `4.75` ≈ A4; `5` = C5. |

Power of Two uses approximately `16.3516 × 2^Value` Hz. Equal Tempered counts semisounds instead: `48` semisounds and `4` octaves both give C4.
...
**Carrier Phase (cycle, 0-1)** sets the oscillator's initial position: `0` = 0°, `0.25` = 90°, `0.5` = 180°, and `1` = 360°. Leave it at `0` initially. Its effect can be inaudible in an isolated sine sound and more apparent when signals combine. For phase that changes during a sound, use **PHASE_MOD** in section 18.
...
Loudness sets the intended overall level. Actual playback level also depends on the spectrum, overlapping sounds, processing, and playback equipment.

**Listen checkpoint — softer and louder.** In your first-sound copy, set Bottom Loudness to `32` and run. Change only Loudness to `64` and run again. At the same playback volume, the second should be stronger. If the difference is unclear, check the newest filenames, disable player volume leveling, and confirm that you changed Loudness rather than an envelope scale. Restore `45`.

## 13. Work with envelopes

### Edit, duplicate, and reuse library envelopes 

**Start:** a fresh first-sound copy. In **Envelope Library**, select an envelope, then a node. Type **X value** and **Y value** for exact coordinates, or drag the node. Right-click to insert/remove interior nodes or change segment behavior.

**Listen checkpoint — change the decay.** Run the original sound first. Duplicate Envelope 1 with **Duplicate Envelope**. Select the new row and note its number. Change the middle node from `(0.5, 1)` to `(0.2, 1)`. In **Spectrum sp1 > Partial 1**, point EnvLib to that number, Scale `1`, and Run. The peak should arrive about one second into the five-second sound, followed by a longer decay. If unchanged, check Partial 1's envelope number. Restore that reference to Envelope `1` afterward.
...
| **Fixed** | Preserves the specified length of a segment; thin line. |

The envelope-list menu also provides **Copy Envelope** and **Paste Envelope**. A library edit affects every field referencing that number: in the completed five-sound project, Envelope 1 controls both amplitude and position. Duplicate a curve and change one field's reference to vary only that use. Before deleting a curve, check references and subsequent curve numbers.

<!-------------------------
		This makes no sense to me. Sever
...
When an exercise says **constant envelope `5`**, it means an envelope whose *value* stays at 5, not library entry 5:

1. Beside that parameter, click **Insert Function** or **fn**, then choose **EnvLib**.
2. Set **Envelope Number** to `2` (or your flat envelope's actual number).
...
### Build an envelope with MakeEnvelope

**MakeEnvelope** creates an envelope without editing Envelope Library. Use it for anenvelope whose coordinates call other functions (ex. Random).
...
Each group of three lines is a node. **Add Node**, **ins**, and **rm** add, insert after this node, and remove node. Keep X values increasing; interpolation and length behavior apply to the segment leaving a node. The last node has no outgoing segment and the user can enter a Scalin Factor for the entire envelope.

Always check the receiving field's units: the same curve can describe amplitude, position, probability, etc.

## 14. Build richer spectra

### Add and remove partials

Run the first-sound copy before adding partials. Keep Bottom s1's frequency at **Continuum > Hertz = 440**. Open **Spectrum sp1**, keep **Deviation = 0**. Starting from the first-sound's one partial
, click **Add Partial** twice so that you have three rows. In each row, click **fn**, choose **EnvLib**, and enter the following Envelope Number and Scaling Factor:
...
**Listen checkpoint — a richer sound.** Run the three-partial version and compare it with the one-partial version at the same 440 Hz base frequency. The added frequencies change the sound color; listen for a less pure sound rather than three separate notes. If it still sounds like one sine wave, check that Number of partials is `3`, all three EnvLib rows are filled, and Generate Spectrum is empty. Give a partial a different envelope to make that component enter or fade at a different time.

The row controls let you insert or remove partials. For ordinary numeric counts, LASSIE updates **Number of partials** when the list changes. Do not **Add Partial** and increase the count, but leave required partial rows blank.

**Deviation = 0** keeps the partials at whole-number multiples of the base frequency. A **Deviation = 1** allows each partial to get a value betwee half way to the lower neighbor to half way to theupper neighbor; this is a random procedure and the user can not control the result. Try small changes and compare using the same seed. The first partial, the fundamental, remains unchanged.

### Generate Spectrum

**Generate Spectrum** is not available at the present time.
...
**Listen checkpoint — generated spectrum.** Run with Generate Spectrum filled, then clear that field and run to return to your explicit partial rows. Compare sound color. If changing the row scales seems to have no effect, check that Generate Spectrum is empty. Its Distance control currently has no audible effect, so do not use Distance as a success test.
		------------------------->

## 15. Place sounds in space

Double-click **Bottom s1**, find **Spatialization**, click **Insert Function**, and choose **SPA**. Have your Envelope 2 (straight line) ready. Choose **Method** and **Apply To** before entering envelopes: changing either option clears and rebuilds the rows.

### Stereo

For two channels, use an envelope in the 0–1 range. With the usual channel order, `0` is right, `0.5` is centered, and `1` is left.

**Listen checkpoint — position.** Choose **Apply To: Sound**, insert EnvLib Envelope `2`, and set its scale to `0.5`. Run this centered version. Change only the scale to `0`, run and listen for the right speaker, then use `1` and listen for the left. If all three sound the same, check stereo playback and the EnvLib scale. Restore `0.5`. For movement, use Envelope `1`, scale `1`, as in section 10.

### Multi-Pan

Select **Multi-Pan** and **Apply To: Sound** to enter a separate level envelope for each output channel (an arbitrary number of channels). Click **Insert Channel After** to add Channel 2; **Remove This Channel** removes a channel group. Match the number of groups to **Project Properties > Number of Channels**.
...
For an initial configuration, give Theta a constant envelope of `0` and Radius a constant envelope of `0.5`. A moving Theta envelope creates motion around the arrangement. Speaker count and channel order must match your playback setup; two speakers cannot reproduce a surrounding array.
...
**Sound** gives the entire sound one spatial behavior. **Partial** lets its frequency components occupy different positions. Prepare the [three-partial spectrum](#add-and-remove-partials), then choose **Stereo** and **Apply To: Partial**. Use the row's **ins** control until there are three partial rows. Fill all three with EnvLib Envelope `2`, Scale `0.5`, accept the dialog, and run this centered version. Reopen it, change only Partial 1's scale to `0`, accept, and run again.

**Listen checkpoint — one component moves.** Compare the two versions: the fundamental moves right while the upper components stay centered. This can be subtler than moving the whole sound. If every component moves together, check **Apply To: Partial** and that only the first row uses scale `0`.
...
Run a fresh first-sound copy with **Bottom s1 > Reverb** empty and keep its audio. This is the **dry** signal; **wet** means the reverberated signal. Then click **Insert Function** beside Reverb and choose a model before filling its parameters. 

| Function | Controls | When to use it |
| --- | --- | --- |
| **REV_Simple** | Room Size | The size of the room; begin with `0.5`. |
| **REV_Medium** | Reverb Percentage envelope, Hi/Low Spread, Gain All Pass, Delay | Control the balance between hi and low frequencies and the decay. |
...
| Field | Value |
| --- | --- |
| Reverb Percentage |envelope `(2, 0.25)` |
| Hi/Low Spread | `0.5` |
| Gain All Pass | `0.7` |
| Delay | `0.05` seconds |
...
Here a Reverb Percentage envelope of `0.25` means a quarter reverberatedsignal, not the number 25. **Hi/Low Spread** changes the relationship between high- and low-frequency decay. **Gain All Pass** and **Delay** affect the reverberator's internal echoes and diffusion. Compare small changes individually.

**Listen checkpoint — Medium reverb.** Run these settings and compare the end with the dry file. Listen for the added decay. If it is missing, check that Delay is `0.05` rather than `0` and that Reverb Percentage uses Envelope `2`, Scale `0.25`.
...
Each gain list must contain **exactly six** values. Keep these modest values while learning. A Delay of `0` disables the Medium or Advanced effect.
...
## 17. Shape the sound with filters
...
| --- | --- | --- |
| **LPF** | Low-pass filter | Reduces frequencies above the cutoff. |
...
| **LSF** | Low-shelf filter | Boosts or attenuates the lower frequency region. |
| **HSF** | High-shelf filter | Boosts or attenuates the upper frequency region. |
| **PBEQ** | Peaking band equalizer | Boosts or attenuates a band around the selected frequency. |
...
Keep the frequency positive and below 15000 Hz. Clear **Filter(Optional)** to remove the local filter, and check ancestor filters if filtering remains.

**Listen checkpoint — hear the filter.** Run the three-partial version with LPF at `1000`, then clear Filter(Optional) and run the unfiltered version. The filtered sound should have less upper-frequency content. If the difference is small, check that the spectrum really has the 880 and 1320 Hz partials; filtering the original 440 Hz sine alone gives a weaker demonstration. Keep the two files and compare sound color at the same playback volume.

## 18. Add sound modifiers

**Start:** a fresh first-sound copy for **each modifier** below. This prevents one test's partials or effects carrying into another. DETUNE additionally needs the [three-partial spectrum](#add-and-remove-partials).

1. Open **Bottom s1 > Modifiers**, click **Add Modifier**, and choose a type.
2. Open **Parameters...** and select **Apply to: SOUND**.
3. Fill the required parameters from the table. For each envelope, choose **Insert Function > EnvLib**, Number `2`, Scale equal to the listed value. For GLISSANDO use MakeEnvelope; DETUNE takes plain numbers.
...
| --- | --- | --- |
| **TREMOLO** | Loudness pulses 5 times/sec | Magnitude envelope `0.01`; Rate envelope `5` Hz. |
| **VIBRATO** | Pitch oscillations 5 times/sec | Magnitude envelope `0.05`; Rate envelope `5` Hz. |
...
| **PHASE_MOD** | A change in sound color from varying oscillator phase | Magnitude envelope `0.1` cycle; Rate envelope `5` Hz. |
| **AMPTRANS** | Extremely brief amplitude changes, grainy  | Magnitude envelope `0.01`; Rate envelope `0.25`; Width envelope `1103` samples. |
| **FREQTRANS** | Extremely brief pitch changes grainy | Magnitude envelope `0.01`; Rate envelope `0.25`; Width envelope `1103` samples. |
| **WAVE_TYPE** | White noise replacing the sine sound | Magnitude envelope `1` for noise; `0` for sine. |

**Listen checkpoint — effect OFF and ON.** Set **Default ON chance** to `0%` and Run. Change only that chance to `100%` and Run again. Compare using the table's listening goal. Begin with TREMOLO, VIBRATO, or WAVE_TYPE for clear contrasts; phase, detune, and transient changes can be subtler. If unchanged, check SOUND and every required parameter. Start the next modifier from a fresh first-sound copy, not from the DETUNE spectrum.

### Tremolo, vibrato, and phase modulation

![Tremolo magnitude and rate envelopes](images/tutorial_sound/27-tremolo.png)

Tremolo and Vibrato magnitude are ia percentage of theoriginal value.  A Rate of `5` means five modulation cycles per second.
...
GLISSANDO multiplies frequency: `1` preserves pitch, `2` doubles it, and `0.5` halves it. Zero points result in a frequency of '0' Hz.

![Detune parameters](images/tutorial_sound/29-detune.png)

SOUND DETUNE uses numeric **Spread**, a percentage, with 0 (no change) and 1 (maxim divergence); **Direction** -1 detunes (diveges) and 1 tunes (converges); **Velocity** a percentage, marks the moment when the change starts. These fields do not take envelopes.

### Transients

![Amplitude Transient magnitude, probability, and width](images/tutorial_sound/30-transient.png)

For AMPTRANS and FREQTRANS, **Rate** is a probability from `0` to `1`, not Hz. **Width** is a positive number of audio samples: `1103` is about 25 milliseconds at 44100 Hz. The example produces irregular disturbances, not a regular oscillation.

### Wave type

![Wave Type parameters](images/tutorial_sound/31-wave-type.png)

Use a straight line envelope for WAVE_TYPE, this is a yes-no situation. Combine noise with **MakeFilter** to restrict its frequency range. Use the **Partial** option below (# 19) to combine sine waves and noise.

To disable any modifier, use its remove control or set Default ON chance to `0%` (option useful in testing). Fill required parameters even for an OFF/ON comparison.

## 19. Apply modifiers to individual partials

**SOUND** applies the modifier's parameters across the sound's partials. **PARTIAL** lets each partial have different values or receive no effect at all.
...
5. In the first partial row, set **Probability** to a straight line envelope of `1`, **Magnitude** to a straight line envelope of `0.3`, and **Rate** to a straigh line envelope of `5`.
6. Leave unused parameters as **N/A**. Width is used only for transients. Keep the other partials’ Probability at **N/A** or use a straight line envelope of `0` to leave them unaffected.
...
## 20. Control modifier probabilities and exceptions

After hearing individual effects at 100%, use theirprobabilities to create variety. The main **Default ON ** probability and **Use ON** probabiity controls display percentages: enter `50` for 50%. Inside a partial's Probability envelope, the equivalent value is `0.5`. A 50% chance applies to each selection; it does not guarantee that exactly half of a five-sound run will use the effect.

### Prepare five separate sounds
...
| Child Start Time /Child Type | PREVIOUS_CHILD_DURATION / `0` |
...
**Sweep** produces an uninterrupted sequence of starttimes and requires the Static Function **PREVIOUS_CHILD_DURATION** as an entry for **Child Start Time**.

Run: five centered one-second sounds should begin at 0, 1, 2, 3, and 4 seconds. This is also the **five separate sounds** setup used in sections 21–22. If they overlap or start at random times, recheck Sweep and both Seconds selections.
...
### Make two modifiers mutually exclusive

Keep the five separate sounds setup and choose **Sampling scope: Per sound**. These rules give each sound either tremolo or vibrato:

1. Add **TREMOLO first**, with **Default ON chance = 50%**. In Parameters, select **SOUND** and use straight line envelopes of `0.01` for Magnitude and `5` for Rate.
2. Add **VIBRATO second**, with **Default ON chance = 100%**. Select **SOUND** and use straight line envelopes of `0.3` for Magnitude and `5` for Rate.
...
**Listen checkpoint — hear the selection rule.** Run the two-row example. Each of the five separate sounds should have either tremolo (loudness pulses) or vibrato (pitch oscillations). To check the exception deterministically, set Tremolo's Default ON chance to `100%` and run: only tremolo should be active. Set it to `0%` and run: only vibrato should be active. If both effects occur on one sound, check row order and Vibrato's exception. Restore Tremolo to `50%`. Then compare **Per sound** with **Once per Bottom event**: the latter gives all five sounds the same selected effect. One Per sound run can also select the same effect five times by chance; try another seed before judging that scope.

## 21. Construct a longer piece
...
| **Density** | Density, Area, Under One | A density scale. |
...
### Children Events Definition

| Method | Behavior | Starting exercise |
| --- | --- | --- |
| **Continuum** | Determines children start times independently.n order or randomly | The Random start times from the beginner example. |
| **Sweep** | Places children in sequence from start time '0': no child can start before the previous child's end. Use PREVIOUS_CHILD_DURATION. | Follow the complete [five separate sounds setup](#prepare-five-separate-sounds). |
| **Discrete** | Builds a 3D matrix (attacks*types*durations) containing different layers while avoiding overlapping events within a layer. Uses EDIs | Refer to **Sieves** and the [worked sieve example](#make-a-sieve-of-allowed-values) before running. |

**Listen checkpoint — sequential timing.** Run the [five separate sounds setup](#prepare-five-separate-sounds). You should hear five one-second sounds in order, unlike the scattered starts of the original piece. Check Sweep and Seconds if the timing is wrong. Adding reverb afterward can make decays overlap even though the start times remain sequential.

![Discrete event definition and child package settings](images/tutorial_sound/35-discrete.png)

In DISSCO, all timings are **relative** to the duration of the parent, they are independent from the duration of the piece. For Continuum and Sweep, explicitly select units:

- **Seconds:** ordinary elapsed seconds relative to the duration of the parent.
...
- **EDUs:** Elementary Displacement Units, subdivisions of the beat. With **60 EDU Per Beat** and **quarter note = 60**, 60 EDUs is one second and 30 EDUs is half a second. Necessary for using Western notation features.
...
### Work with Types and Layers 

**Types** represent categories of sounds that have a common makeup or design (spectrum). **Layers** designate streams of distinct sound sources.
...
Use **Select** or **RandomInt** to choose between Child Types. For eample, if the two displayed types are `0` and `1`, use **RandomInt**, Lower Bound `0`, Upper Bound `1`, in Child Type to choose between them. For a *Select** example, look bellow.

In Discrete mode, the child table includes **Weight**, **Attack Envelope**, **Attack Envelope Scale**, **Duration Envelope**, and **Duration Envelope Scale**. Weight affects type selection. The two envelopes shape preferences over how the allowed attacks and durations are distributed in time. Supply valid library envelope numbers and scales. Different layers can overlap; collision avoidance applies within a layer. Finish a cell edit by pressing Tab or clicking another cell.
...
**Start each example from a fresh `tutorial_sound.dissco` copy.** For pitch experiments, apply the [five separate sounds setup](#prepare-five-separate-sounds), then select **Bottom s1 > Frequency > Equal Tempered** and replace its Value with the stated function. Do not Run with the old Hertz value `440` after switching modes. The Discrete exercise below replaces the timing setup instead.

Where an envelope is required, use a straight line envelope with the stated Scale, unless MakeEnvelope is specified.
...
Indices start at **0**: indices 0, 1, and 2 choose 48, 52, and 55. Update the index range if you remove a list row; index 3 is outside this list. Other functions like  RandomOrderInt or CURRENT_CHILD_NUM could be used; if CURRENT_CHILD_NUM is used, thenumber of choices should be the same as the number of children.

**Listen checkpoint — a restricted pitch set.** Run. All five pitches should be C4, E4, or G4. For a reference by ear, replace Choice Index with `0` and Run for C4, then use `1` and `2` for E4 and G4. Restore RandomInt `0`–`2`. An out-of-range error means the list length and index bounds disagree.

### Shape randomness with Stochos

Stochos can dynamically change a random range through an event.

Choose **Stochos** in Equal Tempered Value and select **Range Distribution**. Click **Add New Node** once for a Min/Max/Distribution group. Click the receiving field before using the bottom **Insert Function** button.

| Field | Value |
| --- | --- |
| Offset | Leave blank; the disabled field defaults to the first group. |
| Min | Straight line envelope `48` |
| Max | Straight line envelope `72` |
| Distribution | MakeEnvelope `(0, 0)` to `(1, 1)`, Linear, Flexible, Scale `1` |

Accept the nested dialogs and Stochos. This linear Distribution gives an even spread. Min/Max envelopes change the range over time.

![Stochos Range Distribution envelopes](images/tutorial_sound/37-stochos.png)

**Listen checkpoint — a pitch range.** Run for five pitches across C4–C6. Check Min/Max scales and Equal Tempered if a pitch is outside that range. Five sounds cannot establish a probability distribution; additional seeds let you explore its variety. Build envelopes for Min/Max that are not straight lines; keep Min at or below Max at any moment in time.
...
**Listen checkpoint — listed choices.** Run. Select now maps those indices to C4, E4, or G4, with repeats allowed. Compare with the range example: intermediate semisound pitches are no longer choices. If another pitch occurs, check Select's list and the three threshold envelopes.

### Make a sieve of allowed values

**Sieves** are logical filters that use modulo and Boolean operations.
...
Start from the five separate sounds setup, switch to Equal Tempered, and choose **ValuePick** in Value. It combines a sieve with envelopes controlling its bounds and weights.
...
Use a fresh copy and the five separate sounds setup, then:
...
Intervals are successive semisound steps: starting at 48, this yields `48, 52, 55, 60`, then returns to `48`. Changing Origin transposes the pattern.
...
A Markov model chooses its next **state** from its current state. Each state has an output value, such as a pitch. Start with the five separate sounds setup.
...
**Listen checkpoint — the Markov cycle.** Run: the five sounds should be C4, G4, C5, C4, G4. If not, check Sweep, model id, and matrix entries. To introduce randomness, change the first row to `0, 0.7, 0.3` and Run again. Keep all entries nonnegative and each row's sum at `1`.
...
| --- | --- |
| **Random** | A random number between Lower and Upper Bound; `0` to `25` seconds for the five-sound piece in section 10. |
| **RandomInt** | A random integer, including the bounds; `48` to `72` for semisound pitches. |
...
| **MakeFilter** | [Seven filter types](#17-shape-the-sound-with-filters). |
...
For this comparison, start from a fresh **first_sound.dissco** copy. In Bottom s1, insert **REV_Simple / Sound / Room Size 0.5** and Run **before replacing it**. Keep this audio, then make the reusable room:
