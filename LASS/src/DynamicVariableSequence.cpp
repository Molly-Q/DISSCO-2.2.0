...
//----------------------------------------------------------------------------//
void DynamicVariableSequence::setSegmentTimeType (int index,
...
//----------------------------------------------------------------------------//
interpolation_type DynamicVariableSequence::getSegmentInterpolationType
                                            (int index)
{
    // assumes a valid index is supplied
...
//----------------------------------------------------------------------------//
void DynamicVariableSequence::setSegmentInterpolationType
                              (int index,
...
Iterator<m_value_type> DynamicVariableSequence::valueIterator()
{
    ...
    // every time
    if (totalTime_ != this->getDuration()
    ...
    return (Iterator<m_value_type>
                (new DynamicVariableSequenceIterator(*interpolators_)));
}
...
void DynamicVariableSequence::generateTimes (m_time_type totalTime)
{
    ...
    // to zero)
    if (dTotalFixedTime > totalTime_
        || (dTotalFixedTime < totalTime_ && dTotalFlexPercent == 0))
    {
...
void DynamicVariableSequence::xml_read(XmlReader *xml, char *tagname)
{
	...
	{
		if(!strcmp(tag->name,tagname) && tag->isClosing)
			break;
		...
		if(!strcmp(tag->name,"dv_type"))
		{
			...
			{
				cout << "OH GOD!  I WAS HOPING FOR dvs BUT I GOT "<<tag->getParamValue("value")<<endl;
			}
		}
	}
	delete tag;
